...
def add_murojaat_line(doc, data):
    ...
    add_paragraph(
        ...
        [
            ...
            (f"{year}-yil {day}-{month}", {'italic': True, 'size': 12}),
...
def add_signature_block(doc):
    ...
    add_run(p, "Markaz direktori:", bold=True)
    p.add_run('\t')
    add_run(p, "S.Mutalibov", bold=True)
    add_paragraph(doc, [], space_after=4)
    add_paragraph(doc, [("Ijrochi: D.Atamirzayev", {'bold': True, 'italic': True, 'size': 9})])
